...
export function Teaching({ heading = 'h2' }: { heading?: 'h1' | 'h2' }) {
  ...
  return (
    <Section id="teaching" tone="deep">
      <Container>
        <div className="grid gap-10 lg:grid-cols-[minmax(0,6fr)_minmax(0,5fr)] lg:items-start lg:gap-24">
          ...
          {spec.length > 0 && (
            <div className="relative border border-line-strong bg-card p-5 md:p-9" data-reveal>
              ...
              <dl className="m-0 mt-6 flex flex-col">
                {spec.map((row) => (
                  <div
                    key={row.label}
                    className="grid grid-cols-[110px_1fr] gap-4 border-t border-line py-3 md:grid-cols-[130px_1fr] md:py-4"
                  >
